                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                                        ...
                                        <?php foreach ($languages as $key => $item) : ?>
                                            <li class="nav-item rounded-0" role="presentation">
                                                <a class="nav-link <?= $key == 0 ? "active" : ""; ?>" id="pills-<?= $item['code']; ?>-tab" data-toggle="pill" href="#pills-<?= $item['code']; ?>" role="tab" aria-controls="pills-<?= $item['code']; ?>" aria-selected="true"><img width="20" class="mr-2" src="<?= public_url('admin/flags/4x3/' . $item['flag']); ?>.svg" alt=""><?= $item['code']; ?></a>
                                            </li>
                                    ...
                                    <div class="tab-content" id="pills-tabContent">
                                        ...
                                        <?php foreach ($languages as $key => $item): $code = $item['code'];?>
                                            <div class="tab-pane fade <?= $key == 0 ? "show active" : ""; ?>" id="pills-<?= $item['code']; ?>" role="tabpanel" aria-labelledby="pills-<?= $item['code']; ?>-tab">
                                                <input name="title[<?= $item['code']; ?>]" type="text" class="form-control" value="<?= $language['title']->$code; ?>">
                                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="code" class="form-control single-select">
                                    <?php foreach (languageList() as $key => $item) { ?>
                                        ...
                                        <option <?= $language['code'] == $key ? 'selected' : ''; ?> value="<?= $key; ?>"><?= lang($key); ?></option>
                                    <?php } ?>
                                </select>
                            </div>
